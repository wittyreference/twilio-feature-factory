...
## Decision 32: Chaos Validation Results — Architect Resilience Baseline

### Context

The generative chaos validation plan (D31) was executed across 3 difficulty levels (mild, moderate, extreme) with 21 total scenarios generated by an LLM chaos agent. Each scenario combined 1-3 failure categories (vague input, wrong products, missing requirements, conflicting requirements, scope mismatch, beginner misconceptions, toolchain edge cases) across 7 developer archetypes. The goal was to establish a baseline for how the toolchain handles realistic bad input and identify systemic weaknesses.
...
Two confirmed weaknesses warrant targeted fixes:

1. **H3 (No scope gate) — CONFIRMED across 4 scenarios**: The architect engages with oversized requests (full contact center, production-hardened POC, 400-template BXML converter) rather than explicitly flagging "this is too big for one session." It silently restructures scope (phasing, prerequisites) without telling the developer their ask is unreasonable. Fix: add explicit scope check to architect prompt.

2. **H2 (Excluded API cross-reference) — PARTIALLY CONFIRMED**: One scenario proposed Conversations API (excluded/EOL) as a Chat replacement; another engaged with Flex as if available. Subsequent runs avoided this. Fix: add excluded API list reference to architect prompt context.

Three hypotheses remain untested because the architect catches everything:
- H1 (Spec resolves ambiguity): untested — no scenario reached spec
- H5 (console.error invariant): untested — no scenario reached code generation
- H6 (Duplicate detection): disproved — the architect found and referenced existing implementations

### Alternatives Considered

- **Weaken the architect to test downstream phases**: Rejected — deliberately passing bad input to spec/dev to test their resilience would require a separate "cascade testing" mode, not a modification to the main chaos plan
- **Increase scenario count for statistical significance**: Rejected for now — 21 scenarios across 3 difficulty levels with consistent trends is sufficient for a baseline. Future runs will accumulate more data.
- **Fix scope gate via orchestrator instead of architect**: Rejected — the architect is the right place because it evaluates the request before any pipeline work begins. The orchestrator has time/budget limits but no semantic understanding of scope.
...
| 2026-03-22 | D43 | Value leakage detection: post-commit hook (<400ms) detects unmapped syncable files, /value-audit runs hybrid-parallel adversarial review (Advocate + Critic as independent Opus subagents → Arbiter synthesis). Parallel > sequential to prevent anchoring bias. Assessment artifacts route to .meta/ (meta-safety preserved). |
