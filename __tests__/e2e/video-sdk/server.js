// ABOUTME: Local Express server for Video SDK E2E tests.
// ABOUTME: Serves the test harness and generates access tokens for browser clients.

const path = require('path');
const express = require('express');

// Load environment variables from project root .env
require('dotenv').config({ path: path.resolve(__dirname, '../../../.env') });

const twilio = require('twilio');
const AccessToken = twilio.jwt.AccessToken;
const VideoGrant = AccessToken.VideoGrant;

const PORT = process.env.VIDEO_SDK_TEST_PORT || 3334;

const app = express();

// Serve the test harness
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'harness.html'));
});

// Serve the Video SDK browser bundle from node_modules
app.get('/sdk/twilio-video.min.js', (req, res) => {
  res.sendFile(path.resolve(__dirname, '../../../node_modules/twilio-video/dist/twilio-video.min.js'));
});

// Serve audio fixture files for transcription tests
app.use('/fixtures', express.static(path.join(__dirname, 'fixtures')));

// Generate access tokens for the browser client
app.get('/api/token', (req, res) => {
  const accountSid = process.env.TWILIO_ACCOUNT_SID;
  const apiKey = process.env.TWILIO_API_KEY;
  const apiSecret = process.env.TWILIO_API_SECRET;

  if (!accountSid || !apiKey || !apiSecret) {
    const missing = [];
    if (!accountSid) { missing.push('TWILIO_ACCOUNT_SID'); }
    if (!apiKey) { missing.push('TWILIO_API_KEY'); }
    if (!apiSecret) { missing.push('TWILIO_API_SECRET'); }
    return res.status(500).json({
      error: `Missing env vars: ${missing.join(', ')}`
    });
  }

  const identity = req.query.identity || `video-user-${Date.now()}`;
  const roomName = req.query.room;

  const videoGrant = new VideoGrant({
    room: roomName
  });

  const token = new AccessToken(accountSid, apiKey, apiSecret, { identity });
  token.addGrant(videoGrant);

  res.json({
    token: token.toJwt(),
    identity,
    room: roomName || null
  });
});

// Only start listening if run directly (not imported by tests)
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`Video SDK test server running at http://localhost:${PORT}`);
    console.log('Open the URL above to use the video harness.');
  });
}

module.exports = { app, PORT };
