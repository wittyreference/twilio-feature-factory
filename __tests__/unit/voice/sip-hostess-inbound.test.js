// ABOUTME: Unit tests for sip-hostess-inbound ConversationRelay handler.
// ABOUTME: Verifies TwiML output, recording setup, and relay URL handling.

const Twilio = require('twilio');
global.Twilio = Twilio;

describe('sip-hostess-inbound', () => {
  let handler;

  beforeAll(() => {
    handler = require('../../../functions/voice/sip-hostess-inbound').handler;
  });

  test('returns ConversationRelay TwiML when relay URL is configured', (done) => {
    const context = {
      SIP_HOSTESS_RELAY_URL: 'wss://test.ngrok.dev',
    ...
    handler(context, event, (err, twiml) => {
      ...
      expect(xml).toContain('Google.en-US-Neural2-C');
      done();
    });
  });
